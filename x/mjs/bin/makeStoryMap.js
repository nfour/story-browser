...
const { dryRun: IS_DRY_RUN, output: OUTPUT = "./storyMap.json", from: FROM = process.cwd(), _: PATTERNS, } = Yargs(hideBin(process.argv)).command("makeStoryMap <patterns...>", "Make a story map by matching glob patterns", (y) => {
    return y
        .option("output", {
